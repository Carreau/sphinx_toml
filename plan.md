...
bugs, dead code, and no safety net. The most urgent items are correctness bugs
that break on specific Python versions or on a direct import of `models`.

---

## P0 — Correctness bugs (fix first)

1. **`models.py` typo `Optinal[str]` → `Optional[str]`** (line ~24).
   Because of `from __future__ import annotations` this does not fail on bare
   import, but it raises `PydanticUndefinedAnnotation: name 'Optinal' is not
   defined` as soon as the `Config` model is built or validated (verified with
   `Config.model_rebuild()`). It is silent only because the models are currently
   unused — it will break P1 #4 immediately. Also remove the **duplicate**
   `today_fmt` field (declared on two lines).

2. **`Normaliser.normalise` missing `self`.** The base class method signature is
...
**Verification:** `python -c "import sphinx_toml"` succeeds, and
`python -c "from sphinx_toml.models import Config; Config.model_rebuild()"`
succeeds after the fix (it fails before). Test under 3.10 and 3.11 if possible.
...
4. **Actually use the `models.Config`.** Today the Pydantic models are dead
...
5. **Migrate to Pydantic v2 idioms.** Replace `class Config: extra = Extra.forbid`
...
6. **Reconcile schema with normaliser output.** The models describe the *input*
...
7. **Replace `print(...)` with `logging`.** The loader prints on every build.
...
8. **Make the `sphinx.toml` path configurable / robust.** It is hard-coded to
...
9. **Guard the environ side effect.** `[sphinx_toml.environ]` mutates
...
10. **Add a test suite.** Create `tests/` with:
    - an import smoke test (would have caught P0 #1),
...
11. **Add CI.** A GitHub Actions workflow running the test suite on Python
    3.10 / 3.11 / 3.12 — this would surface the 3.11 tomli bug automatically.

12. **Add linting/formatting.** `ruff` (lint + format) with a minimal config;
    it flags the unused imports, the undefined-name typo, and the missing `self`.

13. **Expand the README.** Document every supported `sphinx.toml` section with a
    complete example file, and describe the environ and intersphinx-registry
    features. Add a `CHANGELOG`.

14. **Metadata polish.** The `pyproject.toml` MIT classifier and `LICENSE`
...
- **Milestone 1 (bugs):** P0 #1–#3 + a smoke test (P3 #10 partial) + CI (P3 #11).
  This makes the package correct and keeps it correct.
- **Milestone 2 (validation):** P1 #4–#6, modernizing to Pydantic v2 and turning
  the models into a real schema check.
- **Milestone 3 (polish):** P2 #7–#9 and remaining P3 items.
